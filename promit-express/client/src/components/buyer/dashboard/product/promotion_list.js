...
export default function PromotionCard(props) {
  ...
  const imgClicked = () =>{
    ...
    let curPromotion = props.promotion;
    curPromotion.rating = states[1].rating;

    props.setCurrentFunnel(curPromotion,states[1].rating);
  ...
  const getOverAllRating = () => {
    ...
    if (!states[2]) {
      axios.get(`/api/product/rating/${props.promotion.product_id}`).then((res) => {
  ...
  return (
    ...
    <Card className={classes.root}>
      ...
      {/* <a href={`http://localhost:5000/api/image/${props.promotion.picture_id}`} target="_blank"> */}
        <CardMedia
