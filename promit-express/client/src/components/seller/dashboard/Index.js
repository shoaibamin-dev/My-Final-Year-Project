...
export default function Dashboard(props) {
  ...
  const loadSells = () => {
    ...
    console.log("loadsells", states[2].current_seller_data.products_sold);

    if (!sells.isLoaded) {
      // console.log("length",Object.keys(states[2].current_seller_data.products_sold).length)
      let joined_sold_products = [];
      let p_sold = states[2].current_seller_data.products_sold;
      let oc = 0;
      let te = 0
        ...
        for (var property in p_sold) {
          if (p_sold.hasOwnProperty(property)) {
            ...
            axios.get(`/api/product/${elem[1]}`).then(product_result => {
              ...
              if (Object.keys(states[2].current_seller_data.products_sold).length == oc + 1) {
          ...
          {(states[0] == "Dashboard") ? (
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={4} lg={3}>
                <Paper className={fixedHeightPaper}>
                  <Deposits totalEarnings={sells.totalEarnings} />
                </Paper>
              </Grid>
              {/* Recent Orders */}
              <Grid item xs={12}>
